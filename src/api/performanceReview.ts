...
const fetchGetReview = async (id: string) => {
	return authedJSON<reviewData>(
		`${BACKEND_SRT_API}/api/v1/performances/${id}/reviews`,
		{
			method: "GET",
...
const fetchPostReview = async (id: string, content: string, star: number) => {
	return authedJSON(`${BACKEND_SRT_API}/api/v1/performances/${id}/reviews`, {
		method: "POST",
		headers: { "Content-Type": "application/json" },
...
const fetchPatchReview = async (reviewId: string, content: string) => {
	return authedJSON(`${BACKEND_SRT_API}/api/v1/reviews/${reviewId}`, {
		method: "PATCH",
		headers: { "Content-Type": "application/json" },
